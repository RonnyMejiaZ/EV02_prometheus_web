...
public class PerfilRepository {
    private static final Map<Long, Perfil> DATA = new LinkedHashMap<>();
    private static final AtomicLong SEQ = new AtomicLong(1);

    public static synchronized List<Perfil> findAll() {
        return new ArrayList<>(DATA.values()); // copia defensiva
    }

    public static synchronized Perfil save(Perfil p) {
    ...
    // ----- BÚSQUEDA -----
    public static synchronized List<Perfil> search(String q) {
        if (q == null || q.isBlank())
            return findAll();
    ...
    // ----- Paginación -----
    public static PagedResult<Perfil> searchPage(String q, int page, int size) {
        List<Perfil> all = search(q);
        int from = Math.max(0, Math.min(page * size, all.size()));
        int to = Math.min(from + size, all.size());
        List<Perfil> slice = all.subList(from, to);
        return new PagedResult<>(slice, all.size(), page, size);
    }

    public static synchronized Perfil update(Perfil p) {
        p.setUpdatedAt(LocalDateTime.now());
        DATA.put(p.getId(), p);
        return p;
    }

    public static synchronized Perfil findById(long id) {
        return DATA.get(id);
    }
