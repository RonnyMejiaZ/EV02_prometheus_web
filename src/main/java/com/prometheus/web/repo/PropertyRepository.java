...
public class PropertyRepository {
    private static final Map<Long, Property> DATA = new LinkedHashMap<>();
    private static final AtomicLong SEQ = new AtomicLong(1);

    public static synchronized List<Property> findAll() {
        return new ArrayList<>(DATA.values()); // copia defensiva
    }

    public static synchronized Property save(Property p) {
    ...
    // ----- BÚSQUEDA -----
    public static synchronized List<Property> search(String q) {
        if (q == null || q.isBlank())
            return findAll();
        String term = q.toLowerCase(Locale.ROOT).trim();
        return DATA.values().stream()
                .filter(p -> contains(p.getNombre(), term) ||
                        contains(p.getDireccion(), term) ||
                        contains(p.getDescripcion(), term))
    ...
    // ----- Paginación -----
    public static PagedResult<Property> searchPage(String q, int page, int size) {
        List<Property> all = search(q);
        int from = Math.max(0, Math.min(page * size, all.size()));
        int to = Math.min(from + size, all.size());
        List<Property> slice = all.subList(from, to);
        return new PagedResult<>(slice, all.size(), page, size);
    }

    public static synchronized Property update(Property p) {
        p.setUpdatedAt(LocalDateTime.now());
        DATA.put(p.getId(), p);
        return p;
    }

    public static synchronized Property findById(long id) {
        return DATA.get(id);
    }
}
